...
export default function AgendaCalendario({ citasIniciales, medicos, boxes, pacientes }: Props) {
  ...
  const handleAction = async (formData: FormData) => {
    ...
    if (user?.id) formData.set('usuarioId', user.id);
  ...
  const getCitaColor = (estado: string) => {
    const colors: Record<string, { bg: string; text: string }> = {
      PROGRAMADA: { bg: 'var(--primary-light)', text: 'var(--primary-color)' },
      EN_CURSO: { bg: 'var(--color-en-curso-bg)', text: 'var(--color-en-curso-text)' },
      PENDIENTE_PAGO: { bg: '#fef3c7', text: '#92400e' },
      COMPLETADA: { bg: 'var(--color-completada-bg)', text: 'var(--color-completada-text)' },
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', gap: '1.5rem', fontSize: '0.8rem', padding: '0.75rem', backgroundColor: 'var(--bg-card)', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
            {[
              { label: 'Programada', color: 'var(--primary-light)', border: 'var(--primary-color)' },
              { label: 'En Curso', color: 'var(--color-en-curso-bg)', border: 'var(--color-en-curso-text)' },
              { label: 'Pendiente Pago', color: '#fef3c7', border: '#92400e' },
              { label: 'Completada', color: 'var(--color-completada-bg)', border: 'var(--color-completada-text)' },
              { label: 'Cancelada', color: 'var(--color-cancelada-bg)', border: 'var(--color-cancelada-text)' },
            ].map(l => (
              <div key={l.label} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontWeight: 500, color: 'var(--secondary-color)' }}>
                <span style={{ display: 'inline-block', width: '12px', height: '12px', borderRadius: '50%', backgroundColor: l.color, border: `2px solid ${l.border}` }}></span> {l.label}
              </div>
            ))}
          </div>
        ...
        <button onClick={() => { setSelectedSlot({ inicio: toLocalISO(new Date()) }); setModalMode('NEW_CITA'); setErrorMsg(null); }} style={{ padding: '0.75rem 1.5rem', backgroundColor: 'var(--primary-color)', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)' }}>
      ...
      <Card title="Calendario Semanal" subtitle="Haz clic en un bloque de 30 minutos para agendar una cita al instante">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: `80px repeat(${days.length}, minmax(150px, 1fr))`, gap: '1px', backgroundColor: 'var(--border-color)', border: '1px solid var(--border-color)', borderRadius: '8px', overflow: 'hidden', minWidth: 'max-content' }}>
          ...
          {days.map((d, dayIdx) => {
            ...
            return (
              <div key={d} style={{ backgroundColor: isToday ? 'var(--primary-light)' : 'var(--bg-app)', color: isToday ? 'var(--primary-color)' : 'inherit', padding: '10px', textAlign: 'center', fontWeight: 'bold', borderBottom: '1px solid var(--border-color)', borderTop: isToday ? '3px solid var(--primary-color)' : 'none' }}>
                <div>{d}</div>
          ...
          {hours.map(h => (
            <React.Fragment key={h}>
              <div style={{ backgroundColor: 'var(--bg-app)', padding: '10px', textAlign: 'right', fontSize: '0.85rem', color: 'var(--secondary-color)', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', borderBottom: '2px solid var(--border-color)', borderRight: '1px solid var(--border-color)' }}>
                {h}:00
              </div>
              {days.map((_, dayIdx) => {
                ...
                const renderSubSlot = (citas: Cita[], mins: number) => (
                  <div style={{ flex: 1, minHeight: '45px', borderBottom: mins === 0 ? '1px dashed var(--border-color)' : 'none', padding: '4px', cursor: 'pointer', transition: 'background-color 0.2s ease', display: 'flex', flexDirection: 'column', gap: '4px' }}
                    onMouseEnter={(e) => e.currentTarget.style.backgroundColor = 'var(--color-slot-hover)'}
                    onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}
                    onClick={(e) => { if (e.target === e.currentTarget) handleCellClick(dayIdx, h, mins); }}
                  >
                     {citas.map(c => {
                ...
                return (
                  <div key={dayIdx} style={{ backgroundColor: 'var(--bg-card)', borderLeft: '1px solid var(--border-color)', borderBottom: '2px solid var(--border-color)', display: 'flex', flexDirection: 'column' }}>
            ...
            <form action={handleAction} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <label className="form-label" style={{ fontSize: '0.8rem', fontWeight: 600 }}>{isNewPatient ? 'Datos del Nuevo Paciente' : 'Paciente Existente'}</label>
                  <span style={{ fontSize: '0.75rem', color: 'var(--primary-color)', cursor: 'pointer', fontWeight: 500 }} onClick={() => setIsNewPatient(!isNewPatient)}>
          ...
          <div style={{ backgroundColor: 'var(--bg-card)', padding: '2rem', borderRadius: '16px', width: '95%', maxWidth: '500px', boxShadow: 'var(--shadow-lg)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
              <div>
                ...
                <span style={{ display: 'inline-block', padding: '2px 8px', borderRadius: '12px', fontSize: '0.75rem', fontWeight: 600, marginTop: '0.5rem', backgroundColor: getCitaColor(selectedCita.estado).bg, color: getCitaColor(selectedCita.estado).text }}>
                  {selectedCita.estado}
                </span>
              </div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'flex-end', gap: '0.5rem', marginTop: '1.5rem', borderTop: '1px solid var(--border-color)', paddingTop: '1.25rem' }}>
              {selectedCita.estado !== 'CANCELADA' && (
                <button onClick={() => handleUpdateEstado(selectedCita.id, 'CANCELADA')} className="btn btn-danger btn-sm" disabled={isPending}>Cancelar Cita</button>
              )}
              {selectedCita.estado === 'PROGRAMADA' && new Date(selectedCita.fechaHoraInicio) < new Date() && (
                <button onClick={() => setModalMode('REPROGRAMAR_CITA')} style={{ padding: '0.5rem 1rem', borderRadius: '8px', border: '1px solid #d97706', color: '#d97706', backgroundColor: '#fef3c7', fontWeight: 600, cursor: 'pointer', fontSize: '0.875rem' }}>Reprogramar Cita Vencida</button>
              )}
              {selectedCita.estado === 'PROGRAMADA' && (
                <button onClick={() => handleUpdateEstado(selectedCita.id, 'EN_CURSO')} style={{ padding: '0.5rem 1rem', borderRadius: '8px', border: '1px solid var(--color-en-curso-text)', color: 'var(--color-en-curso-text)', backgroundColor: 'var(--color-en-curso-bg)', fontWeight: 600, cursor: 'pointer', fontSize: '0.875rem' }} disabled={isPending}>Marcar En Curso</button>
              )}
              {(selectedCita.estado === 'PROGRAMADA' || selectedCita.estado === 'EN_CURSO') && (
                <button onClick={() => handleUpdateEstado(selectedCita.id, 'COMPLETADA')} style={{ padding: '0.5rem 1rem', borderRadius: '8px', border: 'none', color: 'white', backgroundColor: 'var(--color-completada-text)', fontWeight: 600, cursor: 'pointer', fontSize: '0.875rem', boxShadow: '0 1px 2px rgba(5, 150, 105, 0.2)' }} disabled={isPending}>Finalizar</button>
              )}
            </div>
          </div>
        </div>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(15, 23, 42, 0.6)', backdropFilter: 'blur(4px)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'fadeIn 0.2s ease-out' }}>
          <div style={{ backgroundColor: 'var(--bg-card)', padding: '2rem', borderRadius: '16px', width: '95%', maxWidth: '500px', boxShadow: 'var(--shadow-lg)' }}>
            ...
            <form action={async (formData: FormData) => {
              ...
              if (localStart < new Date()) { setErrorMsg("No se pueden agendar citas en horarios pasados."); return; }
              const res = await reprogramarCita(selectedCita.id, localStart.toISOString(), localEnd.toISOString());
              if (!res.success) { setErrorMsg(res.message || 'Error al reprogramar'); addToast(res.message || 'Error al reprogramar', 'error'); }
              else { setModalMode('NONE'); addToast(res.message || 'Cita reprogramada exitosamente', 'success'); }
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem', marginTop: '1rem', borderTop: '1px solid var(--border-color)', paddingTop: '1.25rem' }}>
                ...
                <button type="submit" className="btn btn-primary" disabled={isPending}>{isPending ? 'Validando...' : 'Confirmar Nuevo Horario'}</button>
              </div>
            </form>
          </div>
        </div>
